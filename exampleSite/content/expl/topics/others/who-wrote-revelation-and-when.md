---
title: "Who wrote Revelation, and when?"
themes: ["authorship", "dating"]
weight: 4
prev: /expl/topics/others/the-rapture
next: /expl/topics/others/judgment-in-the-book-of-revelation
docType: expl
appl: /appl/topics/others/reading-of-revelation-the-big-tribulation-and-the-rapture
sources: 
    - pages: 27–47
      ref: beale_rev
---

Every article on this site assumes an author and a date for the book of Revelation, even when it doesn't say so out loud. Both are disputed among serious, faithful scholars — so it's worth stating plainly what this site assumes, why, and what would actually change if you assumed something else instead.

## Who wrote it

<a name="a1f2"></a>
The text itself only says the author's name was {{% bible val="John" link="rev:1,1" lang="en" %}}, a servant on Patmos who received the vision. It doesn't say which John.

The traditional answer, going back to the early church, is John the Apostle — the same John behind the Gospel and the letters that bear his name. That identification is old and widely held, but not unanimous: some scholars, ancient and modern, point to real differences in Greek style and grammar between Revelation and the Gospel of John, and propose a different, otherwise little-known figure called "John the Elder," mentioned in passing by the early bishop Papias. Others simply leave the question open — the book's authority doesn't depend on resolving it, any more than Hebrews' does on identifying its anonymous author.

This site follows the traditional identification (John the Apostle) without treating it as beyond question.

## When it was written

<a name="c4e7"></a>
This one matters more for how the book gets read, because two dates are seriously defended, about thirty years apart:

- **The majority view: around AD 95–96**, under the emperor Domitian. This rests mainly on the early church historian Irenaeus, writing about a century later but close enough to the events to be taken seriously, who says John saw his visions "toward the end of Domitian's reign."
- **A minority view: the mid-to-late 60s AD**, under Nero or just after. This rests on internal clues rather than external testimony — chiefly {{% bible val="the instruction to measure the temple" link="rev:11,1-2" lang="en" %}}, which some read as implying the Jerusalem temple was still standing (it was destroyed in AD 70), and {{% bible val="the count of kings, of whom five have fallen, one is, and one is yet to come" link="rev:17,9-11" lang="en" %}}, which some map directly onto a list of Roman emperors ending at Nero.

This site follows the majority Domitianic date.

## Why it matters here — and what would change

<a name="9b6e"></a>
This isn't just an academic footnote. A handful of articles on this site depend on which date you pick, and it's worth being honest about that rather than pretending the choice is neutral.

- **The seven letters.** {{% int_link val="Ephesus having lost its first love" link="/expl/content/letters/the-letter-to-the-church-in-ephesus" %}}, and {{% int_link val="Laodicea's settled, self-sufficient wealth" link="/expl/content/letters/the-letter-to-the-church-in-laodicea" %}} — rebuilt after a real earthquake without asking Rome for help — both read most naturally as churches with a couple of decades of history behind them, not a few years. A Neronian date would mean rethinking that "complacency" reading in more than one of these articles.
- **Identifying "the harlot" and her kings.** {{% int_link val="The count of seven kings in Revelation 17" link="/expl/content/harlot/who-is-the-harlot-babylon-part-1" %}} only works once you've picked a starting point on the list of emperors. Whichever date this site (or any reader) settles on effectively answers that question too, whether it says so explicitly or not.
- **The shape of the persecution in view.** The pressure described across the letters — {{% int_link val="temples built to honor the emperor" link="/expl/content/letters/the-letter-to-the-church-in-pergamon" %}}, worship demanded province-wide — fits Domitian's reign, when emperor worship was pushed hard across Asia Minor, better than Nero's, whose persecution was sharp but largely confined to Rome itself.
- **666 and Nero.** This one doesn't actually force a choice either way: {{% int_link val="the gematria linking 666 to Nero Caesar" link="/expl/content/beasts/666-the-number-of-the-beast" %}} works under both dates, since the legend that Nero would return kept circulating for decades after his death.

None of this is presented to settle the debate — a reader who finds the Neronian case more persuasive on {{% bible val="the temple still standing" link="rev:11,1-2" lang="en" %}} has real textual ground to stand on. It's here so that if you notice this site assuming long-settled churches and empire-wide imperial pressure, you know why, and you know it's a choice, not an accident.
